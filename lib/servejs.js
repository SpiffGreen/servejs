...
class Servejs {
    ...
    render(req, res, filePath, stObj = {}) {
        try {
            const content = fs.readFileSync(path.join(this.STATIC_FOLDER, Mimeof(filePath) === "application/octet-stream" ? filePath + ".html" : filePath), { encoding: "utf-8" });
            /**
    ...
    include(filePath, stObj = {}) {
        ...
        return this.checkNodecode(content);
    }
